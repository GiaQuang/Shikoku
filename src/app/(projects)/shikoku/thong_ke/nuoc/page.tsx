...
import { api_get_chart_nuoc, api_get_cpu_info, api_get_list_name_nuoc, api_xuat_du_lieu } from '../../api';
...
function BieuDoNuoc({ f_chart_1, f_chart_2, tieu_thu, tien }: { f_chart_1: Function, f_chart_2: Function, tieu_thu: {}, tien: {} }) {
    ...
    const xuat_du_lieu = async () => {
        ...
        if (object_1 === undefined)
            res = await api_xuat_du_lieu({ type: 'nuoc', data: object_2, tieu_thu: tieu_thu, tien: tien });
        else
            res = await api_xuat_du_lieu({ type: 'nuoc', data: object_1, tieu_thu: tieu_thu, tien: tien });
        toast.dismiss();
        if (res == undefined) {
            toast.error('Lỗi kết nối đến server');
            setBtnFree(true);
...
function BieuDoTronLuongNuoc({ data }: {
    data: {
        name1: string,
        name2: string,
        value1: number,
        ...
        series: [
            {
                    ...
                    normal: {
                        ...
                        label: {
                            ...
                            formatter: function (params: any) {
                                ...
                                params.dataIndex;
                                params.value;
                                return params.value.toLocaleString() + ' m3';
                            },
                        },
                    },
                },
                ...
                data: [
                    ...
                    { value: data.value2, name: data.name2, itemStyle: { color: '#0F0' } },
                ]
            }
        ]
    };
    return <div className='w-full h-full flex flex-col text-lg rounded-md p-2 bg-[#8922a3]'>
        <div className='flex flex-row w-full items-center gap-2'>
            <IoWaterOutline style={{ fontSize: 60 }} />
            <div className='pl-2 w-full text-center text-3xl mt-2'>LƯỢNG NƯỚC TIÊU THỤ</div>
        </div>
        <div className={`flex-1 h-full w-full mt-3`}>
            <ReactECharts
        ...
        series: [
            {
                ...
                data: [
                    { value: data.value1, name: data.name1, itemStyle: { color: '#001061AA' } },
                    { value: data.value2, name: data.name2, itemStyle: { color: '#0F2' } },
                ]
            }
        ]
    };
    return <div className='w-full h-full flex flex-col text-lg rounded-md p-2 
            bg-gradient-to-br  to-[#8922a3] from-[#539241]'>
        <div className='flex flex-row w-full items-center gap-2'>
...
export default function Page() {
    ...
    const f_chart_1 = (data: { category: never[], value: never[], name: string }) => {
        ...
        setTien({
            ...
            name2: data.name,
            value1: 0,
            value2: Number.parseFloat((data.value.reduce((a, b) => a + b, 0) * dien_nuoc.gia_nuoc).toFixed(0)),
        })
    }
    ...
    const f_chart_2 = (data: { category: never[], value1: never[], value2: never[], name1: string, name2: string }) => {
        ...
        setTien({
            name1: data.name1,
            name2: data.name2,
            value1: Number.parseFloat((data.value1.reduce((a, b) => a + b, 0) * dien_nuoc.gia_nuoc).toFixed(0)),
            value2: Number.parseFloat((data.value2.reduce((a, b) => a + b, 0) * dien_nuoc.gia_nuoc).toFixed(0)),
        })
    }
    return (
        <div
            className={`text-white text-3xl font-bold w-full h-screen flex flex-col p-2 gap-2 pt-4
                `}
        >
            <div className={`m-1 text-center`}>TÍNH TOÁN CHI PHÍ TIÊU THỤ NƯỚC</div>
            <div className='flex-1 grid grid-rows-3 w-full h-full gap-3'>
                <div className="grid grid-cols-2 gap-3">
                    <div className="rounded-md ring-2 ring-[#0c0] h-full w-full"><BieuDoTronLuongNuoc data={data} /></div>
                    <div className="rounded-md ring-2 ring-[#fff] h-full w-full"><BieuDoTronChiPhi data={tien} /></div>
                </div>
                <div className="row-span-2"><BieuDoNuoc f_chart_1={f_chart_1} f_chart_2={f_chart_2} tieu_thu={data} tien={tien} /></div>
            </div>
        </div>
    );
}
